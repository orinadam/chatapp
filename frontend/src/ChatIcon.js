...
const ChatIcon = (props) => {
  ...
  const deleteChat = async () => {
    ...
    await chatsActions.deleteChats("/chats/", props.chatId);
  };
  ...
  return (
    <Flex w="100%" border="1px solid #BDBDBD" cursor="pointer">
      <Box
        ...
        cursor="pointer"
        _hover={{ background: "#E0E0E0", color: "teal" }}
        padding={1}
        h={50}
        w="90%"
        ...
        onClick={handleClick}
      >
        <Heading paddingTop={1.5} fontSize={17} marginLeft={2}>
          {props.username}
        </Heading>
        <Text fontSize="10" marginLeft={4} color="gray.600" isTruncated>
          {props.lastMessage}
        </Text>
      </Box>
      <Spacer />
      <DeleteIcon
        ...
        marginTop={3}
        cursor="pointer"
        _hover={{ background: "#E0E0E0", color: "red.700" }}
        color="gray.400"
        onClick={deleteChat}
      />
    </Flex>
